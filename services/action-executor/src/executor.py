...
class ActionExecutionEngine:
    ...
    def __init__(self) -> None:
        ...
        # both saw no existing record and both executed the remediation.
        self._key_locks: Dict[str, asyncio.Lock] = {}
        self._key_locks_guard = asyncio.Lock()

    async def _lock_for(self, idempotency_key: str) -> asyncio.Lock:
        """Returns the lock guarding one idempotency key, creating it once."""
    ...
    async def execute_approved_action(
        ...
        # Held across the check and the write so the two are one step.
        async with await self._lock_for(idempotency_key):
            # 1. Idempotency Check
