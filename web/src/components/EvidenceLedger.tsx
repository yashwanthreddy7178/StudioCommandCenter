...
export const EvidenceLedger: React.FC<EvidenceLedgerProps> = ({ events }) => {
  ...
  return (
    <div className="bg-studio-surface border border-studio-border rounded-xl p-5 shadow-panel flex flex-col h-full">
      <div className="flex items-center justify-between pb-3 border-b border-studio-border/60">
        ...
        <span className="text-[11px] font-mono text-studio-fg3">Stream: SSE Live</span>
      </div>

      <div className="mt-4 flex-1 overflow-y-auto space-y-3 max-h-[480px] pr-1">
        {events.length === 0 ? (
          <div className="text-center py-12 text-studio-fg4 font-mono text-xs">
              ...
              >
                ...
                >
                  <div className="flex items-start space-x-2.5">
                    <span className="bg-studio-bg text-studio-fg3 px-1.5 py-0.5 rounded text-[10px] font-bold">
                      #{event.seq}
                    </span>
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="font-semibold text-studio-fg">{event.title}</span>
                        {toolName && (
                        ...
                        {isCacheHit && (
                          <span className="bg-studio-accent/20 text-studio-accent border border-studio-accent/30 text-[9px] px-1.5 py-0.5 rounded font-bold flex items-center space-x-1">
                            <Zap className="w-2.5 h-2.5" />
                            <span>CACHE HIT</span>
                          </span>
                        )}
                      </div>
                      <p className="text-studio-fg2 text-xs mt-1 font-sans leading-relaxed">
                        {event.description}
                      </p>
                    </div>
                  </div>

                  <button className="text-studio-fg3 hover:text-studio-fg p-1">
                ...
                {isExpanded && event.payload && (
                  <div className="mt-3 pt-3 border-t border-studio-border/50">
                    <div className="bg-studio-bg rounded p-2.5 text-[11px] text-studio-fg2 overflow-x-auto max-h-48 border border-studio-border/40">
